...
// Tests that TEX token supply control mechanisms operate correctly.
contract("TEX", function ([
    _,
    admin,
    newSupplyController,
    otherAddress,
    ...
    beforeEach(async function () {
        // const TEX = await TEX.new({ from: owner });
        // const proxy = await Proxy.new(TEX.address, { from: admin });
        // const proxiedTEX = await TEX.at(proxy.address);
        // await proxiedTEX.initialize({ from: owner });
        // this.token = proxiedTEX;
        // Token config
        this.decimals = 18;
        this.symbol = "TEX";
    ...
    describe("as a supply-controlled token", function () {
        ...
        describe("decreaseSupply", function () {
            ...
            describe("when the supply controller has insufficient tokens", function () {
                it("reverts", async function () {
                    await assertRevert(
                        this.token.decreaseSupply(decreaseAmount, { from: owner })
                    );
                });
            });
            ...
            describe("when the supply controller has sufficient tokens", function () {
                ...
                it("reverts when sender is not supply controller", async function () {
                    await assertRevert(
                        this.token.decreaseSupply(decreaseAmount, { from: otherAddress })
                    );
                });
                ...
                it("emits a SupplyDecreased and a Transfer event", async function () {
                    const { logs } = await this.token.decreaseSupply(decreaseAmount, {
                        from: owner
                    });
        ...
        describe("setSupplyController", function () {
            ...
            beforeEach(async function () {
                const res = await this.token.setSupplyController(newSupplyController, {
                    from: owner
                });
                logs = res.logs;
            });
            ...
            it("reverts if sender is not owner or supplyController", async function () {
                await assertRevert(
                    this.token.setSupplyController(otherAddress, { from: otherAddress })
                );
            });

            it("works if sender is supply controller", async function () {
                await this.token.setSupplyController(otherAddress, {
                    from: newSupplyController
                });
            ...
            it("reverts if newSupplyController is address zero", async function () {
                await assertRevert(
                    this.token.setSupplyController(ZERO_ADDRESS, { from: owner })
                );
            });
